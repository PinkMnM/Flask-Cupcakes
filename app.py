...
if app.config["SQLALCHEMY_DATABASE_URI"] is None:
    app.config["SQLALCHEMY_DATABASE_URI"] = get_database_uri(
        "cupcake", cred_file=None, save=False
    )
...
def get_cupcakes():
    ...
    return jsonify({"cupcakes": [cupcake.to_dict() for cupcake in Cupcake.query.all()]})
...
if __name__ == "__main__":
    if len(sys.argv) >= 3:
        app.config["SQLALCHEMY_DATABASE_URI"] = get_database_uri(
            sys.argv[3] if len(sys.argv) > 3 else "cupcake", sys.argv[1], sys.argv[2]
        )
